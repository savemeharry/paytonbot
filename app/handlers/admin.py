...
async def cmd_toggle_channel(message: types.Message):
    ...
    async with get_session(session_factory) as session:
        ...
        channel = await session.scalar(
            f"SELECT is_active FROM channels WHERE id = {channel_id}"
        )
        ...
        try:
            ...
            await session.execute(
                f"UPDATE channels SET is_active = {new_status} WHERE id = {channel_id}"
            )
            await session.commit()
...
async def cmd_add_tariff(message: types.Message):
    ...
    async with get_session(session_factory) as session:
        ...
        try:
            ...
            # Check if channel exists
            channel = await session.scalar(
                f"SELECT id FROM channels WHERE id = {channel_id}"
            )
    ...
    async with get_session(session_factory) as session:
        ...
        try:
            ...
            user = await session.scalar(
                f"SELECT id FROM users WHERE user_id = {target_user_id}"
            )
            ...
            tariff_query = await session.execute(
                f"SELECT id, duration_days FROM tariffs WHERE id = {tariff_id}"
            )
        ...
        try:
            ...
            try:
                channel_tg_id = await session.scalar(
                    f"SELECT channel_id FROM channels WHERE id = {channel_id}"
                )
...
async def cmd_del_sub(message: types.Message):
    ...
    async with get_session(session_factory) as session:
        ...
        try:
            ...
            await session.execute(
                f"UPDATE subscriptions SET is_active = 0 WHERE id = {sub_id}"
            )
            await session.commit()
